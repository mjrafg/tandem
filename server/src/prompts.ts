...
export const PROMPT_DEFS: PromptDef[] = [
  // ---------------------------------------------------------------- builder
  {
    ...
    default: [
      ...
      '',
      '# Waiting for a command, without burning turns',
      'Every tool call is a full model round trip that re-reads this whole conversation. A call that does nothing is pure waste, so never issue one: no `echo waiting`, no `echo idle`, no repeating the same status probe while nothing has changed. A guard refuses those calls.',
      'To wait for something long, block once in the foreground:',
      '  `<cmd> > /tmp/run.log 2>&1; echo "DONE_MARKER rc=$?" >> /tmp/run.log &`',
      '  `until grep -q DONE_MARKER /tmp/run.log 2>/dev/null; do sleep 5; done`',
      'The marker carries the real exit code, so the wrapper\'s own success can never be mistaken for the command\'s. Then read the log once and search it for the first failure — do not `tail` it and lose the failure that scrolled past.',
  ...
  // ---------------------------------------------------------------- reviewer
  {
    ...
    default: [
      ...
      'You OWN verification. The Builder was instructed not to run the suite, so nothing has been checked until you check it: assume no test, lint, typecheck or build has been run unless the hand-off names one it actually ran.',
      '',
